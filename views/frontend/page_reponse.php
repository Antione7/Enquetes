...
<div class="container">
    ...
    <div class="row">
        <form action="../../controllers/frontend/page_reponse_process.php?id=<?php echo $id; echo isset($unique_id)?"&unique_id=".$unique_id:null ?>" method="POST">
        <?php
        ...
        foreach ($tab_question as $question) :?>
            ...
            <div class="block_question">
                <fieldset class="block">
                    ...
                    <textarea class="form-control" name="texte<?php echo $currentQuestion ?>"><?php echo isset($reponse)?$reponse->getLibelle():null ?></textarea>
                    ...
                    <input class="form-control" type="text" name="nombre<?php echo $currentQuestion ?>" value="<?php echo isset($reponse)?htmlentities($reponse->getLibelle()):null ?>" />
                    <?php break;
                    case 'qcm': 
                    ...
                    foreach ($tab_choix as $choix): ?>
                        <label class="radio radio_reponse">
                            <input type="radio" name="qcm<?php echo $currentQuestion ?>" value="<?php echo htmlentities($choix->getLibelle()) ?>" <?php if(isset($reponse)){
                                echo $choix->getLibelle()==$reponse->getLibelle()? "checked":null;
                                } ?> /><?php echo strtoupper($tab_letter[$i]) ?>:&nbsp;<?php echo htmlentities($choix->getLibelle()) ?>
                        </label>
                        <?php $i++ ?>
                    <?php endforeach; ?>
        ...
        <?php endforeach; ?>
            <hr/>
            <input class="btn btn-success pull-right" type='submit' value='envoyer'/>
        </form>
    </div>
</div>
